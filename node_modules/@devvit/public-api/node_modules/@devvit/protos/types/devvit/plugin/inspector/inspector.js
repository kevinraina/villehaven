...
export const InspectorServiceName = "devvit.plugin.inspector.Inspector";
export class InspectorClientImpl {
    constructor(rpc, opts) {
        this.service = opts?.service || InspectorServiceName;
        this.rpc = rpc;
        this.ListActors = this.ListActors.bind(this);
        this.ActorChanges = this.ActorChanges.bind(this);
    }
    ListActors(request, metadata) {
        const data = InspectorQuery.encode(request).finish();
        const promise = this.rpc.request(this.service, "ListActors", data, metadata);
        return promise.then((data) => InspectorDescriptions.decode(_m0.Reader.create(data)));
    }
    ActorChanges(request, metadata) {
        const data = Empty.encode(request).finish();
        const result = this.rpc.serverStreamingRequest(this.service, "ActorChanges", data, metadata);
        return result.pipe(map((data) => InspectorUpdate.decode(_m0.Reader.create(data))));
    }
}
