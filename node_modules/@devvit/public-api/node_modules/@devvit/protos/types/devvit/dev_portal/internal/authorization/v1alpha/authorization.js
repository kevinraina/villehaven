...
export const DevPortalAuthorizationServiceName = "devvit.dev_portal.internal.authorization.v1alpha.DevPortalAuthorization";
export class DevPortalAuthorizationClientImpl {
    constructor(rpc, opts) {
        this.service = opts?.service || DevPortalAuthorizationServiceName;
        this.rpc = rpc;
        this.CanViewLogs = this.CanViewLogs.bind(this);
    }
    CanViewLogs(request, metadata) {
        const data = DevPortalAuthCanViewLogsRequest.encode(request).finish();
        const promise = this.rpc.request(this.service, "CanViewLogs", data, metadata);
        return promise.then((data) => DevPortalAuthorizationResponse.decode(_m0.Reader.create(data)));
    }
}
