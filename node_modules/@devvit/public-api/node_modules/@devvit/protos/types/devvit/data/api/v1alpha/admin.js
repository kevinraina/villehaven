...
export const DataApiAuthTokenIssuerServiceName = "devvit.data.api.v1alpha.DataApiAuthTokenIssuer";
export class DataApiAuthTokenIssuerClientImpl {
    constructor(rpc, opts) {
        this.service = opts?.service || DataApiAuthTokenIssuerServiceName;
        this.rpc = rpc;
        this.GenerateAuthToken = this.GenerateAuthToken.bind(this);
    }
    GenerateAuthToken(request, metadata) {
        const data = DataApiAuthTokenRequest.encode(request).finish();
        const promise = this.rpc.request(this.service, "GenerateAuthToken", data, metadata);
        return promise.then((data) => DataApiAuthTokenResponse.decode(_m0.Reader.create(data)));
    }
}
