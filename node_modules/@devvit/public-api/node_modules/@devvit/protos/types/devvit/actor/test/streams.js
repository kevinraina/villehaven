...
export const StreamsServiceName = "devvit.actor.test.Streams";
export class StreamsClientImpl {
    constructor(rpc, opts) {
        this.service = opts?.service || StreamsServiceName;
        this.rpc = rpc;
    ...
    Transform(request, metadata) {
        ...
        const result = this.rpc.bidirectionalStreamingRequest(this.service, "Transform", data, metadata);
        return result.pipe(map((data) => TestMessage.decode(_m0.Reader.create(data))));
    }
    Aggregate(request, metadata) {
        const data = request.pipe(map((request) => TestMessage.encode(request).finish()));
        const promise = this.rpc.clientStreamingRequest(this.service, "Aggregate", data, metadata);
        return promise.then((data) => TestMessage.decode(_m0.Reader.create(data)));
    }
    Enumerate(request, metadata) {
        const data = TestMessage.encode(request).finish();
        const result = this.rpc.serverStreamingRequest(this.service, "Enumerate", data, metadata);
        return result.pipe(map((data) => TestMessage.decode(_m0.Reader.create(data))));
    }
}
