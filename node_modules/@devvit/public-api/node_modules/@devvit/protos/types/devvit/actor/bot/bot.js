    ...
    fromJSON(object) {
        return {
            commands: globalThis.Array.isArray(object?.commands)
                ? object.commands.map((e) => CommandDescription.fromJSON(e))
                : [],
        };
    },
    toJSON(message) {
...
export const BotServiceName = "devvit.actor.bot.Bot";
export class BotClientImpl {
    ...
    GetCommands(request, metadata) {
        const data = Empty.encode(request).finish();
        const promise = this.rpc.request(this.service, "GetCommands", data, metadata);
        return promise.then((data) => CommandsList.decode(_m0.Reader.create(data)));
    }
    OnCommand(request, metadata) {
        const data = CommandRequest.encode(request).finish();
        const promise = this.rpc.request(this.service, "OnCommand", data, metadata);
